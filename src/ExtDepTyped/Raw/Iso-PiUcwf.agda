...
-- open import Unityped.PiUcwf
open import ExtDepTyped.Raw.ExpSub renaming (Tm to Tm-cwf ; Sub to Sub-cwf ; q to q⋆ ; p to p⋆ ; id to id⋆ ; _∘_ to _∘⋆_ ; _[_] to _[_]⋆)
open import ExtDepTyped.Raw.ImpSub renaming (Tm to Tm-λ ; Sub to Sub-λ) hiding (subComp ; idExt ; p-∘ ; cong-∘₁ ; subLam ; subΠ)
...
wk-⟦⟧ (t [ γ ]⋆)
  rewrite sym (wk-sub-p {t = ⟦ t ⟧ [ ⟦ γ ⟧' ]})    = refl~βη
wk-⟦⟧ (Π A B) rewrite sym $ wk-sub-p {t = ⟦ A ⟧}
                    | sym $ /-↑-[] {t = ⟦ B ⟧}     = refl~βη
