module Unityped.Wellscoped.Typed.ScwfInt where
...
data Tm where
  ...
  _[_] : ∀ {m n} {Γ : Ctx n} {Δ : Ctx m} {α} → Tm Γ α → Subst Γ Δ → Tm Δ α
  app : ∀ {n α β} {Γ : Ctx n} → Tm Γ (α ⇨ β) → Tm Γ α → Tm Γ β
  lam : ∀ {n α β} {Γ : Ctx n} → Tm (Γ , α) β → Tm Γ (α ⇨ β)
