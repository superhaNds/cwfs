------------------------------------------------------------------------------------
-- An initial Ucwf with extra structure in which everything is explicit
...
-- of explicit substitutions that is a ucwf by construction. This should be an
-- object in the category of λβη-ucwfs.
------------------------------------------------------------------------------------

module Unityped.ExpSub.UcwfExpSub where
...
------------------------------------------------------------------------------------
-- Mutually recursive terms and substitutions as data types

-- Terms are indexed by the maximum number of free variables they may contain

data Tm : Nat → Set

-- A substitution from m to n is a length n list of terms with at most m free variables
...
------------------------------------------------------------------------------------
-- The equality relations for Terms and Substitutions
...
data _≈_  where
  ...
  cong-sub : ∀ {m n} {t u} {ts us : Sub m n}
             → t ≈ u
             → ts ≋ us
             → t [ ts ] ≈ u [ us ]
...
data _≋_ where
  ...
  assoc : ∀ {m n k p} (ts : Sub n k) us (vs : Sub p m)
          → (ts ∘ us) ∘ vs ≋ ts ∘ (us ∘ vs)
  ...
  sym≋ : ∀ {m n} {ts : Sub m n} {us : Sub m n}
         → ts ≋ us
         → us ≋ ts
              
  trans≋ : ∀ {m n} {ts us vs : Sub m n}
           → ts ≋ us
           → us ≋ vs
           → ts ≋ vs
...
------------------------------------------------------------------------------------
-- The pair (Tm, Sub) form a ucwf, λ-ucwf, and λ-βη-ucwf
...
open Ucwf Tm-Ucwf using (setoidT ; setoidS)
...
TmSetoid {n} = setoidT {n}

SubSetoid : ∀ {n m} → Setoid _ _
SubSetoid {n} {m} = setoidS {n} {m}

cong-sub₁ : ∀ {n m} {t₁ t₂ : Tm n} {ts : Sub m n}
            → t₁ ≈ t₂
            → t₁ [ ts ] ≈ t₂ [ ts ]
cong-sub₁ eq = cong-sub eq refl≋

cong-sub₂ : ∀ {n m} {t} {ts₁ ts₂ : Sub m n}
            → ts₁ ≋ ts₂
            → t [ ts₁ ] ≈ t [ ts₂ ]
cong-sub₂ = cong-sub refl≈

cong-∘₁ : ∀ {m n k} {ts₁ ts₂ : Sub m n} {vs : Sub k m}
          → ts₁ ≋ ts₂
          → ts₁ ∘ vs ≋ ts₂ ∘ vs
cong-∘₁ eq = cong-∘ eq refl≋

cong-∘₂ : ∀ {m n k} {ts₁ ts₂ : Sub k m} {vs : Sub m n}
          → ts₁ ≋ ts₂
          → vs ∘ ts₁ ≋ vs ∘ ts₂
cong-∘₂ = cong-∘ refl≋ 

cong-<, : ∀ {m n} {ts₁ ts₂ : Sub m n} {t}
          → ts₁ ≋ ts₂
          → < ts₁ , t > ≋ < ts₂ , t >
cong-<, = cong-<,> refl≈           

cong-,> : ∀ {m n} {ts : Sub m n} {t u}
          → t ≈ u
          → < ts , t > ≋ < ts , u >
cong-,> eq = cong-<,> eq refl≋          
...
qLift₂ : ∀ {m n k} (s : Sub m n) (t : Sub k (suc m))
         → q [ (⇑ s) ∘ t ] ≈ q [ t ]
qLift₂ s t = begin
  q [ (⇑ s) ∘ t ]                  ≈⟨ refl≈ ⟩
