...
record Ucwf (Term : Nat → Set) : Set₁ where
  ...
  field
    ...
    p∘sub  : ∀ {m n k : Nat} → {t : Term n} → {ts : Hom n k} → comp n (suc k) k (p k) (⟨_,_⟩ n k ts t) ≡ ts
...
record λβ-ucwf (Term : Nat → Set) : Set₁ where
  ...
  field
    ...
    β   : {n m : Nat} {t : Term (suc n)} {u : Term n} → app n (lam n t) u ≡ subs uc n (suc n) t (⟨_,_⟩ uc n n (id uc n) u)
