...
record Ucwf (Term : Nat → Set) : Set₁ where
  ...
  field
    ...
    ∘asso    : ∀ {m n k p : Nat} (ts : Hom n k) (us : Hom m n) (vs : Hom p m)
                 → p , m , k ⟨ (m , n , k ⟨ ts ∘ us ⟩) ∘ vs ⟩ ≡ p , n , k ⟨ ts ∘ (p , m , n ⟨ us ∘ vs ⟩)⟩
    subid    : ∀ {m n : Nat} (t : Term n) → (n , n ▹ t [ id n ]) ≡ t
    p∘<γ∘α>  : ∀ {m n k : Nat} → {t : Term n} → {ts : Hom n k} → n , (suc k) , k ⟨ p k ∘ (n , k < ts , t >)⟩ ≡ ts
    q[<γ,α>] : ∀ {m n : Nat} {ts t} → n , (suc m) ▹ (q m) [ n , m < ts , t > ] ≡ t
    ∘sub     : ∀ {m n : Nat} → ∀ {t ts us} → m , n ▹ t [ m , n , n ⟨ ts ∘  us ⟩ ] ≡ m , n ▹ (n , n ▹ t [ ts ]) [ us ]
    <δ,α>∘γ  : ∀ {m n : Nat} → ∀ {t ts us} → m , n , suc m ⟨ (n , m < ts , t >) ∘ us ⟩ ≡ m , m < m , n , m ⟨ ts ∘ us ⟩ , (m , n ▹ t [ us ]) >
...
record λβ-ucwf (Term : Nat → Set) : Set₁ where
  ...
  field
    ...
    β   : {n : Nat} {t : Term (suc n)} {u : Term n} → app n (lam n t) u ≡  n , (suc n) ▹ t [ n , n < id n , u > ]
    η   : {n : Nat} {t : Term (suc n)} → lam n (app (suc n) t (q n)) ≡ {!!} -- needs Term n but t ∈ Term (suc n)
